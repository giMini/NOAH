...
include_once("configfetch.php");	
	$tsqlInformation = "select count(*) Total FROM [NOAH].[dbo].[DNSCacheAudited] tableInf, [NOAH].[dbo].[ServerAudited] sa, [NOAH].[dbo].[Hunt] hu
		WHERE 
	...
	$tsqlInformation = "SELECT 						
		hu.huntingGUID, sa.serverName, sa.serverID, DNSCacheAuditedID, [RecordName]
	  FROM [NOAH].[dbo].[DNSCacheAudited] tableInf, [NOAH].[dbo].[ServerAudited] sa, [NOAH].[dbo].[Hunt] hu
		WHERE 
		tableInf.serverID = sa.serverID
		AND hu.huntingID = sa.huntingID
		$sqlFilter		
		GROUP BY hu.huntingGUID, sa.serverName, sa.serverID, DNSCacheAuditedID, [RecordName]
		ORDER BY DNSCacheAuditedID
...
if(sqlsrv_has_rows($getInformation)) { 							
	while( $row = sqlsrv_fetch_array( $getInformation, SQLSRV_FETCH_ASSOC)) {  
		echo '<tr><td>'.$row['huntingGUID'].'</td><td>'.$row['serverName'].'</td><td>'.$row['RecordName'].'</td></tr>';  										
	}  
}
